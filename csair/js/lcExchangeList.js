...
$(function() {
	...
	$(".lcRadio:eq(0)").prop("checked",true);
//*******************判断页面加载是否成功****************************//
...
//点击列表单选框
	$(".lcRadio").on("click",function(){
		$(".lcRadio").each(function(i){
			$(this).prop("checked","");
		})
...
//里程兑换查询
	...
	$("#next").click(function(){
		$(".lcRadio").each(function(i){
			var lcRadioChecked = $(this).prop("checked");
			if(lcRadioChecked == true){
...
function cuXiaoChangGuiLiChengCheck(e){
	...
	if(LiChengIdBeChecked != cgLiChengIdBeChecked){
		...
		$.ajax({
			url : 'loadLcFlightInfo?'+timestamp,
			type : 'GET',
			data:{"LiChengIdBeChecked":LiChengIdBeChecked},
			dataType : 'html',
			sync : false,
			success : function(data) {
				$(".flight_sel_result").html(data);
				$(".lcRadio:eq(0)").prop("checked",true);
				var a = $("li.flight_sel_result_price a");
				var length = $("li.flight_sel_result_price a").length;
				for(i=0;i<length;i++){
