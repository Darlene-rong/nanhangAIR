			...
			success : function(data) {
				...
				for(var i=0;i<data.reserve.length;i++){
					...
					if(data.reserve[i].zk == null && data.reserve[i].lc == null){
						...
						var nologin=$(".container").find(".nologin");
						if(nologin.length>0){
							src=src+'<a class="pop" title="'+data.reserve[i].name+data.reserve[i].price+'元,点击此处进行明珠会员登录购票" href="'+data.reserve[i].href+'" onclick="pop(this);">明珠会员登录购票</a><a class="orangebtn nologinbuy"  href="'+data.reserve[i].href+'" onclick="nologin(this);" title="点击此处可无需登录购票">无需登录购票&nbsp;</a></li></ul>';
						}else{
							src=src+'<a class="pop" title="'+data.reserve[i].name+data.reserve[i].price+'元,点击此处进行明珠会员购票" href="'+data.reserve[i].href+'" onclick="pop(this);">明珠会员购票</a></li></ul>';
						}
						$(li).closest("ul").find("li.flight_sel_result_price_show_infos").append(src);
					}
				}
...
function nologin(e){
	...
	$.ajax({
		...
		success:function(data){
			if(data.notice =='true'){
			 window.location.href="nologinorder";
			}else{
				alert("旅客信息页面获取失败！");
				$.unblockUI();
				reader.stopAll();
	...
	$.ajax({
		...
		success:function(data){
			if(data.state=="SUCCESS"){
			 window.location.href="toaddpassenger2";
			}else{
				alert("登录失败，请仔细核对帐号和密码");
				$.unblockUI();
				reader.stopAll();
			}
		}
